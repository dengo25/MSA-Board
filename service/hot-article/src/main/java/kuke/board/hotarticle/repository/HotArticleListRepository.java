...
public class HotArticleListRepository {
  ...
  // sorted set으로 저장할텐데 hot-article::list::{yyyyMMdd}
  ...
  public void add(Long articleId, LocalDateTime time, Long score, Long limit, Duration ttl) {
    redisTemplate.executePipelined((RedisCallback<?>) action -> { //executePipelined는 네트워크 연결시 데이터를 한번에 연결시켜준다.
      StringRedisConnection conn = (StringRedisConnection) action; //conn을 이용해 레디스와 통신
      String key = generateKey(time);
      conn.zAdd(key, score, String.valueOf(articleId));
      conn.zRemRange(key, 0, - limit - 1); //상위 10건의 데이터를 sortedSet에 데이터를 유지
